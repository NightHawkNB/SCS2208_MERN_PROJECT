...
const createFines = async(req,res)=>{
    const {user_id,borrow_id}=req.body
    ...
    try{
        const fines = await Fines.create({user_id,borrow_id})
        res.status(200).json(fines)
    }catch(err){
        res.status(404)({error:err.massage})
    }
}
...
const deleteFines = async(req,res)=>{
    ...
    const fines=await Fines.findByIdAndDelete({borrow_id:borrow_id})
